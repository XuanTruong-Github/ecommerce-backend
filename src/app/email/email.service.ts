...
export class EmailService {
  ...
  private async verifyConnection() {
    try {
      ...
      this.logger.info('SMTP connected');
    } catch (error) {
      this.logger.error({
        msg: `SMTP connection failed: ${error}`,
  ...
  async sendVerificationEmail(to: string, name: string, url: string): Promise<boolean> {
    const subject = 'Confirm email';
    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;"> <h2 style="color: #333;">Xác thực email</h2> <p>Xin chào ${name},</p> <p>Cảm ơn bạn đã đăng ký tài khoản! Vui lòng xác thực email để hoàn tất quá trình đăng ký.</p>
